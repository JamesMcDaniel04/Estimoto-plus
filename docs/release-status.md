...
Second round on the same branch: customers now have an **Activity** feed with an unread badge, fed by provider request events, ready estimates, confirmed shop times and due reminders (a five-minute worker scan), and each notice is emailed through the existing Resend account unless the customer turns **Email me about updates** off in Settings. Push notifications remain unimplemented: they need APNs/FCM credentials and a native build, which this environment cannot produce. The Settings Connections card now shows the real Google Calendar state and opens the calendar screen; Gmail is marked coming soon. New garages get a **Getting started** checklist. The app caches the last loaded garage on the device and opens it with an Offline banner when the server is unreachable. Release builds post anonymous, redacted crash reports to `POST /v1/client-errors`, readable by operators through the bridge key. Dark mode follows the system setting, the welcome headline scales safely at large text sizes, and icon-only controls carry labels; light and dark screenshots are under `qa/dark-mode/`. Backend: an IP abuse guard for unauthenticated 401 sprays and the public routes, a 60-second cache on the Android release pointer, bounded `vehicle_id` inputs, and migration `a4c1e7b9d2f0` (customer_notifications, client_errors, customers.notification_emails). The privacy policy describes the notices, emails, diagnostics and on-device cache.

Not closed here, because they are outside the repository: push notification credentials and native builds, localization (all copy remains English), the GitHub Actions billing lock, Apple external beta approval, Google Play publication, Google Places activation, and the first real shop handoff.

Remaining before a public launch: set the service-role secret on Fly, deploy (the release step runs `alembic upgrade head`), resolve the GitHub Actions billing lock so hosted CI runs, obtain Apple external beta approval and Google Play publication, and prove physical-device capture and the first real shop handoff as listed below.
